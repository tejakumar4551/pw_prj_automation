...
- The pipeline MUST be configured to upload the contents of the `reports/` and `allure-results/` folders as CI artifacts if a test fails or completes. Because failure media (screenshots/videos) is stored inside `reports/failure-media/`, capturing `reports/` will automatically capture all visual evidence.
